...
(defn comma-join
  ...
  (str compiles-to "(" (->> args (map identity #_ compile-expression)
                            util/str-commajoin) ")"))
...
(defn single-arg
  ...
  (if (= (count args) 1)
    (str compiles-to "(" (-> args first identity #_ compile-expression) ")")
